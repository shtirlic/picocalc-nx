...
static inline int lcd_spi_cmddata(struct spi_dev_s *dev, uint32_t devid,
                                  bool cmd);

static void picocalc_lcd_select(struct ili9341_lcd_s *lcd);
static void picocalc_lcd_deselect(struct ili9341_lcd_s *lcd);
static int  picocalc_lcd_backlight(struct ili9341_lcd_s *lcd, int level);
...
static int picocalc_lcd_recvparam(struct ili9341_lcd_s *lcd, uint8_t *param);
static int picocalc_lcd_recvgram(struct ili9341_lcd_s *lcd, uint16_t *wd,
                                 uint32_t nwords);
...
static void picocalc_lcd_configure(struct ili9341_lcd_s *lcd, uint8_t cmd,
                                   uint8_t data_bytes, const uint8_t *data)
{
  lcd->select(lcd);
  lcd->sendcmd(lcd, cmd);
  if (data_bytes)
...
static int picocalc_lcd_sendcmd(struct ili9341_lcd_s *lcd, const uint8_t cmd)
{
  struct picocalc_lcd_s *priv = (struct picocalc_lcd_s *)lcd;
      ...
      if (g_lcd != NULL)
        {
          for (int i = 0; i < nitems(g_lcd_config); i++)
            {
              picocalc_lcd_configure(&priv->dev, g_lcd_config[i].cmd,
